...
def mean_dynamic_spectrum(trial_info_path, title, name, **filters):
    ...
    plt.ylabel("Frequency (Hz)")
    plt.title(title)
    plt.savefig('dynamic_spectra/' + name)
    plt.close()
...
def save_to_h5(mouse_name, trial_type, data):
    ...
    """
    filename = f"{mouse_name}_dynamic_spec.h5"

    with h5py.File(filename, "a") as h5f:
        group = h5f.require_group(trial_type)

        group.create_dataset("PowerSpectra", data=data["PowerSpectra"])
        group.create_dataset("TimeSignals", data=data["MeanTimeSignals"])
        group.create_dataset("MeanPowerSpectrum", data=data["MeanPowerSpectrum"])
...
def main(mouse_name, trial_info_file):
    ...
    trial_types = ['HC Hit (3)', 'MC Hit (2)', 'MC Miss (5)', ['HC No Report (9)', 'Incorrect Reject (6)']]
    trial_type_names = ['HC Hit', 'MC Hit', 'MC Miss', 'HC Miss']
    
    # Titles and filenames for saving plots and results
    titles = [
        f"Mean Dynamic Spectrum: {mouse_name} HC Hits",
        f"Mean Dynamic Spectrum: {mouse_name} MC Hits",
        f"Mean Dynamic Spectrum: {mouse_name} MC Miss",
        f"Mean Dynamic Spectrum: {mouse_name} HC Miss"]
    file_names = [
        f"{mouse_name}_hc_hit",
        f"{mouse_name}_mc_hit",
        f"{mouse_name}_mc_miss",
        f"{mouse_name}_hc_miss"]
   ...
   # Open the HDF5 file (create if it doesn't exist)
    ...
    with h5py.File(h5_path, 'a') as h5_file:
        ...
        for i in range(len(trial_types)):
            ...
            data = mean_dynamic_spectrum(trial_info_file, title=titles[i], name=file_names[i], TrialType=trial_types[i])
            save_to_h5(mouse_name, trial_type_names[i], data)

main('cfm004', 'trial_info/TrialInfo_cfm004mjr.csv')
