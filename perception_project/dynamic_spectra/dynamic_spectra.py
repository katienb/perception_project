...
def load_movie(mouse, date, file):
    ...
    noisy_path = "N:/GEVI_Wave/Analysis/Visual/" + mouse + "/20" + str(date) + "/" + file + '/cG_unmixed_dFF.h5'
    denoised_path = "N:/GEVI_Wave/Analysis/Visual/" + mouse + "/20" + str(date) + "/" + file + '/cG_unmixed_dFF_denoised.h5'
...
def mean_dynamic_spectrum(trial_info_path, title, name, **filters):
    ...
    for _, row in unique_recordings.iterrows():
        ...
        try:
            ...
            movie, fps = load_movie(mouse, date, file)
        ...
        for _, trial in df_trials.iterrows():
            try:
                ...
                if (bfm_time - 1)*fps > 0 and (bfm_time + 2)*fps < movie.shape[0]:

                    # Compute frame range for this trial
                    start_frame = int((bfm_time - 1) * fps)
                    end_frame = int((bfm_time + 2) * fps)
                    ...
                    power_norm, frequencies = dynamic_spectrum(mean_signal, fps)
    ...
    mean_power_arr = np.mean(all_power, axis=0)
    all_mean_signals = np.stack([arr[:min_t_length] for arr in all_mean_signals])

    time_arr = np.linspace(-1, 2, mean_power_arr.shape[1])  # Stimulus at t=0]
...
def save_to_h5(mouse_name, trial_type, data):
    ...
    """
    filename = f"results/{mouse_name}_denoised_dynamic_spec.h5"
